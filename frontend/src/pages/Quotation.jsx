...
function Quotation() {
    ...
    if (e.target.value !== "---") {
      ...
      if (found.hasTransducer) {
        ...
        fetch(urlLocal + "getHasTransducer/" + found.idProduct, {
          ...
          .then((data) => {
            if (data) {
              if (data[0].hasTransducer === "YES") setHasTransducer(true);
              else {
  ...
  const handleAddConcept = () => {
    // adds concept to state variable until quotation is done

    var newConcept = {};
    var currentWarranty = "";

    if (selectedWarranty === "") currentWarranty = "Sin garantía";
    else currentWarranty = selectedWarranty;

    console.log(currentWarranty);

    if (selectedWarranty !== "") {
      fetch(
        urlLocal + "getIdWarranty/" + current.idProduct + "/" + currentWarranty,
        {
          method: "GET",
        }
      )
        ...
        .then((data) => {
          if (data) {
            ...
            if (hasTransducer) {
              // if product has transducers available, search index
              const found = transducers.find(
                (element) => element.name === selectedTransducer
              );
              currentIdTransducer = found.idTransducer;
            } else {
              //since it doesn't have compatible transducers, selected transducer will be empty, so we search 'NONE' / 'Ninguno' directly
              const found = transducers.find(
            ...
            newConcept = {
              ...
              idQuotation: 0,
              idWarranty: data[0].idWarranty,
              idTransducer: currentIdTransducer,
  ...
  const handleVerPDF = () => {
    ...
    pdfWindow.document.write(
      "<html><head><title>PDF Document</title></head><body>"
    );
      ...
      <div className={styles.divider}>
        ...
        <form className={styles.form}>
          ...
          <fieldset className={styles.formGroup}>
            ...
            {hasTransducer && (
              <div className={styles.inputGroup}>
                <label htmlFor="transducers" className={styles.label}>
                  Transductores
                </label>
                <select
                  name="transducers"
                  id="transducers"
                  className={styles.select}
                  onChange={handleSaveTransducer}
                >
                  <option selected default>
                    ---
                  </option>
                  {transducers.length !== undefined &&
                    transducers.map((element) => (
                      <option key={element.key} value={element.value}>
                        {element.name}
                      </option>
                    ))}
            ...
            <div className={styles.inputGroup}>
              ...
              <textarea
                ...
                id="description"
                /* cols="30" rows="3" */ placeholder={description}
                title="Contenido no editable"
                readOnly
              ></textarea>
            </div>
            ...
            <div className={styles.inputGroup}>
              ...
              <select
                ...
                id="warranty"
                className={styles.select}
                onChange={handleSaveWarranty}
              >
                <option selected /* value="-1" */>---</option>
                {warranties.length !== undefined &&
                  warranties.map((element) => (
                    <option key={element.key} value={element.value}>
                      {element.name}
                    </option>
                  ))}
        ...
        <form className={styles.form}>
          <fieldset className={styles.formGroup}>
            ...
            <div className={styles.inputGroup}>
              <label htmlFor="shippingCost" className={styles.label}>
                Costo de envío
              </label>
              <input
                type="number"
        ...
        <div className={`${styles.paper}`}>
          <header className={styles.paperHeader}>
            ...
            <div className={styles.paperHeaderGroup}>
              <div className={styles.folioAndDate}>
                <span>
                  FOLIO: <span className={styles.folio}>1337</span>
                </span>
          ...
          <footer className={styles.footer}>
            <div className={styles.footerGroup}>
              <div className={styles.footerGroupItem}>
                ...
                <p>Cotización Contado</p>
              </div>

              <div className={styles.footerGroupItem}>
                <span>GARANT&#205;A:</span>
                <p>
                  .*18 Meses de Garantía en Equipo de Ultrasonido Chison Nuevo
                  (Unidad principal); **12 Meses de Garantía en Transductores
                  Chison Nuevos; ***12 Meses de Garantía en Batería integrada al
                  equipo; ****Impresoras, reguladores y accesorios la garantía
                  es directo con fabricante.
                </p>
              </div>
            </div>
          ...
          >
            ...
            <footer className={styles.footer}>
              ...
              <Link to="/cotizaciones" className={buttonStyles.default}>
                Ver cotizaciones
              </Link>
              <button className={buttonStyles.default} onClick={handleVerPDF}>
                Ver PDF
              </button>
              <PDFDownloadLink
